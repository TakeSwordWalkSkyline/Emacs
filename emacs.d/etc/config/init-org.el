(use-package org
  ...
  (defun custom/capture-word ()
	(interactive)
	(setq-local capture-word-item nil)
	(setq custom/capture-word-data nil)
	...
	(setq custom/capture-word-data (ivy-read "请选择要插入的词性: " capture-word-item))
	(setq custom/capture-word-data (remove "" (split-string custom/capture-word-data "|")))
	(if (null custom/capture-word-data)
		(message "光标下的单词无法捕获!")
	  (org-capture 1 "f")))

  (defun custom/capture-get-word (number)
	(cond ((eq number 1) (nth 0 custom/capture-word-data))
		  ((eq number 2) (nth 1 custom/capture-word-data))
		  ((eq number 3) (nth 2 custom/capture-word-data))))
